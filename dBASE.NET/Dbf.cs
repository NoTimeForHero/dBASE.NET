...
namespace dBASE.NET
{
    ...
    public class Dbf : BaseDbf
    {
        ...
        /// </summary>
        public Dbf(Encoding encoding = null, IEnumerable<DbfField> fields = null) : base(fields)
        {
        ...
        private readonly List<DbfRecord> _records = new();
        ...
        /// <inheritdoc />
        protected override void ReadData(BinaryReader reader, byte[] memoData)
        {
            _records.Clear();
            ...
            while (reader.PeekChar() != 0x1a && reader.PeekChar() != -1)
            {
                try
                {
                    _records.Add(new DbfRecord(reader, header, _fields, memoData, Encoding));
                }
